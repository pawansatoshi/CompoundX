...
def drift_score(current: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    drops = []
    for key in keys:
        old = _f(baseline.get(key), 0.0); cur = _f(current.get(key), 0.0)
        if old > 0: drops.append(max(0.0, (old - cur) / old))
...
def specialist_reliability(records: list[dict[str, Any]], min_samples: int = 30) -> dict[str, Any]:
    ...
    for row in records or []:
        name = str(row.get('specialist', 'unknown')); n = int(_f(row.get('samples')))
        if n < min_samples:
            out[name] = {'weight': 0.5, 'status': 'INSUFFICIENT_HISTORY', 'samples': n}; continue
        brier = _f(row.get('brier'), 0.25); ece = _f(row.get('ece'), 0.10); accuracy = _clamp(row.get('accuracy', 0.5))
...
def calibrated_probability(probability: float, reliability_weight: float = 1.0, uncertainty: float = 0.0) -> dict[str, Any]:
    p = _clamp(probability); w = _clamp(reliability_weight); u = _clamp(uncertainty)
...
def dynamic_risk_size(equity: float, entry: float, stop: float, base_risk: float, *, probability: float, edge: float, regime_confidence: float, liquidity_score: float, execution_score: float, portfolio_multiplier: float = 1.0, degradation_multiplier: float = 1.0) -> dict[str, Any]:
    equity, entry, stop = _f(equity), _f(entry), _f(stop); distance = abs(entry - stop)
    if equity <= 0 or entry <= 0 or distance <= 0: return {'size': 0.0, 'risk_amount': 0.0, 'multiplier': 0.0, 'reason': 'invalid sizing inputs'}
...
def signal_half_life(age_seconds: float, half_life_seconds: float) -> dict[str, Any]:
    age, half = max(0.0, _f(age_seconds)), max(1.0, _f(half_life_seconds, 1.0)); remaining = 2.0 ** (-age / half)
...
def self_critique(*, thesis: str, supporting: list[str], contradicting: list[str], missing: list[str], invalidation: str, probability: float, calibrated_lower_bound: float, net_ev: float | None) -> dict[str, Any]:
    failures = []
    if thesis and not invalidation: failures.append('thesis has no explicit invalidation')
    if supporting and len(contradicting) >= len(supporting) and contradicting: failures.append('contradicting evidence is not dominated')
    if calibrated_lower_bound < 0.55: failures.append('calibrated lower probability bound is weak')
    if net_ev is not None and net_ev <= 0: failures.append('net expected value is not positive')
    return {'passed': not failures, 'failures': failures, 'thesis': thesis, 'invalidation': invalidation, 'probability': round(_clamp(probability), 6), 'calibrated_lower_bound': round(_clamp(calibrated_lower_bound), 6), 'net_ev': None if net_ev is None else round(_f(net_ev), 8)}
...
def adaptive_snapshot(result: dict[str, Any], *, baseline: dict[str, Any] | None = None, specialist_records: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    votes = result.get('votes', []) if isinstance(result, dict) else []; p = _f(result.get('score', result.get('probability', 0.5)), 0.5)
    regime = result.get('regime', {}) if isinstance(result.get('regime'), dict) else {}; current_metrics = result.get('strategy_metrics', {}) if isinstance(result.get('strategy_metrics'), dict) else {}
    drift = drift_score(current_metrics, baseline or {}) if baseline else {'score': 0.0, 'drift': False, 'severe': False, 'action': 'NO_BASELINE'}
    reliability = specialist_reliability(specialist_records or []); uncertainty = _clamp(result.get('uncertainty', 0.0)); calibration = calibrated_probability(p, 1.0, uncertainty)
    return {'regime': regime, 'probability': calibration, 'evidence_dependency': evidence_dependency(votes), 'drift': drift, 'specialist_reliability': reliability, 'state': 'CAUTION' if drift.get('drift') or calibration['lower_bound'] < 0.55 else 'NORMAL', 'learning_policy': 'observe -> attribute -> validate OOS -> promote; never self-enable live trading'}
