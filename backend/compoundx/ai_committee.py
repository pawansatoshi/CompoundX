...
def evaluate_committee(c:dict[str,Any])->CommitteeDecision:
    ...
    he,hd,hr=_history_edge(c); votes.append(Vote('history',1 if he>0 and hd==direction else -1 if he>0 and hd and hd!=direction else 0,he,hr,_q(c,'history'),'historical'))
    ...
    pooled=log_odds_pool([{'model':v.model,'direction':v.direction,'magnitude':v.strength,'quality':v.quality,'cluster':v.cluster} for v in votes]); p=pooled['probability'] if direction>0 else 1-pooled['probability']
    ...
    if direction>0 and p<.60: reasons.append('calibrated probability below threshold')
    if direction<0 and p<.60: reasons.append('calibrated probability below threshold')
