...
def evaluate_committee(context: dict[str, Any]) -> CommitteeDecision:
    ...
    if history_edge < MIN_EDGE:
        reasons.append("historical edge below threshold")
    if history_direction not in (0, direction):
        reasons.append("historical direction conflicts")
    ...
    if rr < MIN_RR:
        reasons.append(f"risk/reward {rr:.2f} below {MIN_RR:.2f}")
    ...
    if not bool(context.get("execution_ok", False)):
