...
def test_quality_pool_decorrelates_clusters():
    r=log_odds_pool([{'model':'trend','direction':1,'magnitude':.9,'quality':1,'cluster':'price'},{'model':'momentum','direction':1,'magnitude':.9,'quality':1,'cluster':'price'},{'model':'flow','direction':-1,'magnitude':.9,'quality':1,'cluster':'flow'}])
    assert len(r['clusters'])==2 and r['conflict'] is True

def test_portfolio_correlation_limits():
    p=build_portfolio([{'symbol':'ETH/USDT','quantity':1,'entry_price':100}],{'ETH/USDT':[1,2,3]}, {'symbol':'BTC/USDT','equity':100,'returns':[1,2,3]})
    assert p['candidate_max_abs_correlation']>0.99 and portfolio_risk_gate(p,.005)['passed'] is False
...
def test_strategy_dispatch_is_real():
    c=[float(100+i*.5) for i in range(100)];h=[x+1 for x in c];l=[x-1 for x in c];v=[100]*100;r=generate(h,l,c,v,'trend_following')
