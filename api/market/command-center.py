...
def _trade_ledger(symbol,limit=100):
    ...
    with connection() as conn:
        cur=conn.execute("SELECT id,symbol,side,quantity,entry_price,exit_price,realized_pnl,fees,slippage,mode,status,opened_at,closed_at FROM trades WHERE symbol=%s ORDER BY opened_at DESC LIMIT %s",(symbol,limit))
        names=[d.name for d in cur.description]; rows=[dict(zip(names,r)) for r in cur.fetchall()]
        op=conn.execute("SELECT symbol,side,quantity,entry_price,status,opened_at FROM trades WHERE status='OPEN' AND mode='PAPER' ORDER BY opened_at DESC LIMIT 50")
        on=[dict(zip([d.name for d in op.description],r)) for r in op.fetchall()]
...
def _apply_macro_gate(result,symbol):
    calendar=fetch_calendar(symbol=symbol,days=3); result['economic_calendar']=calendar
    passed,reasons=committee_calendar_gate(calendar); result['economic_calendar_gate']='PASS' if passed else 'BLOCKED'; result['macro_risk']=calendar.get('risk_score',1.0)
    if not passed:
        result['decision']='NO_TRADE'; result.setdefault('reasons',[]).extend(reasons); result.setdefault('adversarial',{}).setdefault('failures',[]).extend(reasons); result.setdefault('adversarial',{})['passed']=False
    return result
...
def _apply_research(result,ledger):
    ...
    research=build_research_intelligence(market_data={},market_summary=result,trades=ledger.get('trades',[]),feature_names=features)
    result['research_intelligence']=research; result['research_gate']='PASS' if research.get('research_ready') else 'NOT_CERTIFIED'
    if result.get('decision')=='TRADE' and not research.get('research_ready'):
        result['decision']='NO_TRADE'; result.setdefault('reasons',[]).append('research validation not certified')
    return result
...
def _apply_intelligence(result,symbol,market_data,derivatives,expiry,expiry_market_data,equity):
    ...
    # Advisory evidence can influence ranking/confidence, but never blocks merely because it is missing.
    autonomy=build_autonomous_intelligence(symbol=symbol,market_data=market_data,market_summary=result,derivatives=derivatives,expiry=expiry,macro_blocked=result.get('economic_calendar_gate')=='BLOCKED')
    result['autonomous_intelligence']=autonomy; result['autonomous_decision']=autonomy.get('decision','NO_TRADE'); result['live_execution']=False; result['paper_trading']=True
    if autonomy.get('decision')!='TRADE': result['decision']='NO_TRADE'; result.setdefault('reasons',[]).extend(autonomy.get('failures',[]))
    ledger=_trade_ledger(symbol); result['paper_trade_ledger']=ledger
    result=_apply_research(result,ledger)
    portfolio=build_portfolio(ledger.get('open_positions',[]),{}, {'symbol':symbol,'equity':equity}); result['portfolio']=portfolio
    candidate_risk=float(result.get('risk_fraction',0.005) or 0.005); pg=portfolio_risk_gate(portfolio,candidate_risk); result['portfolio_risk_gate']=pg
    if not pg['passed']:
        result['decision']='NO_TRADE'; result.setdefault('reasons',[]).extend(pg['blockers'])
    advtest=adversarial_challenge({'direction':result.get('direction_label',''),'expected_slippage_bps':result.get('expected_slippage_bps',0),'max_slippage_bps':20,'risk_reward':result.get('risk_reward',0),'confidence':result.get('score',0)}, {'macro_risk':result.get('macro_risk',0)}, {'crowding': 'crowded_long' if adv.get('bias',0)<-.3 else 'crowded_short' if adv.get('bias',0)>.3 else 'mixed'}, {'pcr':0})
    result['adversarial_red_team']=advtest
    # Existing adversarial failure remains authoritative; optional derivatives availability does not.
    if not advtest['passed']:
        result['decision']='NO_TRADE'; result.setdefault('reasons',[]).extend(advtest['failures'])
    final_gate=final_derivatives_options_gate(derivatives,expiry,expiry_market_data); result['derivatives_options_gate']=final_gate
    result['strategy_ranking']=rank_strategy(result.get('research_intelligence',{}),portfolio_fit=pg.get('marginal_risk',1.0) or 1.0,regime_fit=float(result.get('autonomous_intelligence',{}).get('regime',{}).get('confidence',1.0) or 1.0))
...
def _scan(symbol,exchange_id,sandbox,equity,limit):
    ...
    derivatives=gateway.derivatives_market_data(symbol); expiry_data=gateway.expiry_market_data(symbol,limit=100)
    ...
    result.update({'exchange':exchange_id,'sandbox':sandbox,'data_source':'exchange_public_market_data','supported_timeframes':gateway.supported_timeframes(),'live_execution':False,'8_percent_daily_target':{'enabled':False,'policy':'research hypothesis only; never a trading target'}})
    return _apply_intelligence(_apply_macro_gate(result,symbol),symbol,market_data,derivatives,expiry,expiry_data,equity)
